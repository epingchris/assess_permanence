...
library(magrittr)
library(mclust)
file_path = paste0("C:/Users/epr26/Documents/carbon_release_pattern/")

# Simulation settings ----

#set simulation type (single sites, portfolio or simulated ones)
type = "project" #expo, expo_portfolio, portfolio, project
scale_c = 1.3  #1.5, 2, 5, 10
...
ggsave(paste0(file_path, file_pref, "_3_aomega_distribution.", file_type), width = 15, height = 15, unit = "cm")
...
if(print_real) {
  ggplot(sim_credit_long) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
      	...
      	axis.text.y = element_text(size = 18, angle = 45),
      	plot.margin = margin(0.7, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, file_pref, "_4a_sim_credit_real.", file_type), width = 15, height = 10, unit = "cm")
}
...
ggplot(summ_credit, aes(x = year)) +
  ...
  theme(axis.line = element_line(linewidth = 0.5),
        ...
        axis.text.y = element_text(size = 18, angle = 45),
        plot.margin = margin(0.7, 1, 0.7, 0.7, "cm"))
ggsave(paste0(file_path, file_pref, "_4_sim_credit_rep.", file_type), width = 15, height = 10, unit = "cm")
...
if(print_real) {
  ggplot(subset(sim_release_long, t <= H)) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
          ...
          axis.text.y = element_text(size = 18, angle = 45),
          plot.margin = margin(0.7, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, file_pref, "_5a_sim_release_real.", file_type), width = 15, height = 10, unit = "cm")
}
...
ggsave(paste0(file_path, file_pref, "_5_sim_release_rep.", file_type), width = 15, height = 10, unit = "cm")
...
if(print_real) {
  ggplot(as.data.frame(sim_ep) %>% replace(., . == 0, NA) %>% mutate(t = row_number()) %>% pivot_longer(V1:V100, names_to = "rep", values_to = "val")) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
      	...
      	axis.text.y = element_text(size = 18, angle = 45),
      	plot.margin = margin(0.7, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, file_pref, "_6a_sim_ep_real.", file_type), width = 15, height = 10, unit = "cm")
}
...
ggplot(summ_ep, aes(x = year)) +
  ...
  theme(axis.line = element_line(linewidth = 0.5),
        ...
        axis.text.y = element_text(size = 18, angle = 45),
        plot.margin = margin(0.7, 1, 0.7, 0.7, "cm"))
ggsave(paste0(file_path, file_pref, "_6_sim_ep_rep.", file_type), width = 15, height = 10, unit = "cm")
...
ggsave(paste0(file_path, file_pref, "_7_sim_cred_rep.", file_type), width = 15, height = 10, unit = "cm")
...
if(transition) {
  ...
  ggplot(data = data_credit, aes(x = var)) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
          ...
          axis.text.y = element_text(size = 18, angle = 45),
          plot.margin = margin(1.5, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, "expo_8a_lc_vs_mean_credit.", file_type), width = 15, height = 10, unit = "cm")
  ...
  ggplot(data = data_max_EP, aes(x = var)) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
          ...
          axis.text.y = element_text(size = 18, angle = 45),
          plot.margin = margin(1.5, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, "expo_8b_lc_vs_max_EP.", file_type), width = 15, height = 10, unit = "cm")
  ...
  ggplot(data = data_mean_cred, aes(x = var)) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
          ...
          axis.text.y = element_text(size = 18, angle = 45),
          plot.margin = margin(1.5, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, "expo_8e_lc_vs_no_cred.", file_type), width = 15, height = 10, unit = "cm")
  ...
  ggplot(data = data_mean_cred, aes(x = var)) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
          ...
          axis.text.y = element_text(size = 18, angle = 45),
          plot.margin = margin(1.5, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, "expo_8e_lc_vs_no_cred_short.", file_type), width = 15, height = 10, unit = "cm")
}


# Transition of simulation results along buffer period value under small 1/lambda_c ----
bp_sensitivity = T

if(bp_sensitivity) {
  scale_c = 1.7
  sub_folder = "bp_sensitivity/"
  expo_text = gsub("\\.", "_", as.character(scale_c))
  bp_vec = c(1:20) #buffer period
  
  mean_credit = matrix(NA, n_rep, length(bp_vec))
  max_EP = matrix(NA, n_rep, length(bp_vec))
  where_max_EP = matrix(NA, n_rep, length(bp_vec))
  final_EP = matrix(NA, n_rep, length(bp_vec))
  mean_cred = matrix(NA, n_rep, length(bp_vec))
  
  for(i_val in 1:length(bp_vec)){
    bp = bp_vec[i_val]
    source(paste0(file_path, "carbon_loss_simulations_core.R"))
  ...
  ggplot(data = data_credit, aes(x = var)) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
          ...
          axis.text.y = element_text(size = 18, angle = 45),
          plot.margin = margin(1.5, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, sub_folder, "expo_", expo_text, "_1_bp_vs_mean_credit.", file_type), width = 15, height = 10, unit = "cm")
  ...
  ggplot(data = data_max_EP, aes(x = var)) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
          ...
          axis.text.y = element_text(size = 18, angle = 45),
          plot.margin = margin(1.5, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, sub_folder, "expo_", expo_text, "_2_bp_vs_max_EP.", file_type), width = 15, height = 10, unit = "cm")
  ...
  ggplot(data = data_mean_cred, aes(x = var)) +
    ...
    theme(axis.line = element_line(linewidth = 0.5),
          ...
          axis.text.y = element_text(size = 18, angle = 45),
          plot.margin = margin(1.5, 1, 0.7, 0.7, "cm"))
  ggsave(paste0(file_path, sub_folder, "expo_", expo_text, "_3_bp_vs_cred.", file_type), width = 15, height = 10, unit = "cm")
}